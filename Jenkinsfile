pipeline {
    ...
    environment {
        APP_NAME = "car-rental-eureka-server"
        RELEASE = "1.0.0"
    ...
    stages {
        ...
        stage("Checkout Eureka server") {
            steps {
                git branch: 'main', credentialsId: 'GITHUB_LOGIN', url: 'https://github.com/myOrganization-GV/car-rental-eurekaServer'
            }
        }
        stage("Build Eureka Application") {
            steps {
        ...
        stage("Build and Push Docker Image") {
            steps {
                script {
                    def jarPath = "target/car-rental-eurekaServer-0.0.1-SNAPSHOT.jar"
            ...
            steps {
                ...
                withCredentials([file(credentialsId: 'GCP_SA_KEY', variable: 'KEYFILE')]) {
                    sh '''
                        ...
                        --zone=${GCP_ZONE} --quiet \\
                        --command "cd ~/car-rental-app && docker compose pull eureka-server && docker compose up -d --no-deps --force-recreate eureka-server"
                    '''
                }
            }
        }
